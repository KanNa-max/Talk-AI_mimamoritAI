...
void init_AImodel() {
    ...
    nnbfile.close();
    Serial.println("[calibration] AI model is loaded.");
}
...
int detec() {
    ...
    for (int y_shift = 0; y_shift < devided; y_shift++) {
        for (int x_shift = 0; x_shift < devided; x_shift++) {
            // 画像の切り取りと縮小処理
            int lefttop_x = CAM_CLIP_X_face + x_shift * PICTURE_CLIP_WIDTH / 2;
            int lefttop_y = CAM_CLIP_Y_face + y_shift * PICTURE_CLIP_HEIGHT / 2;
    ...
    drawBox(imgBuf, CAM_CLIP_X_face, CAM_CLIP_Y_face);
...
void calibration() {
    ...
    init_gesture();
     ...
     Serial.println("[calibration] 枠内を何もない状態にしてください");
    while (count < 5) {
        if (!count_gesture(1)) {
            count++;
        }
    ...
    // gestureのキャリブレーション
     Serial.println("[calibration] 枠内にジェスチャーをしてください");
    while (count < 5) {
        if (count_gesture(2)) {
            count++;
        }
        Serial.print("[calibration] stage2: check your gesture  ");
        Serial.print(count);
        Serial.println("/5");
    }
        Serial.println("[calibration] done!");
    ...
    while (count < 5) {
        ...
        Serial.print("[calibration] stage3: check your face  ");
        Serial.print(count);
        Serial.print("/5  [");
        preview_queue(&queue);
        Serial.println("]");
    ...
    while (count < 5) {
        ...
        Serial.print("[calibration] stage4: check your background  ");
        Serial.print(count);
        Serial.print("/5  [");
        preview_queue(&queue);
        Serial.println("]");
    ...
    while (count < 2) {
        ...
        count = countInQueue(&queue, 0);
        Serial.print("[calibration] stage5: check your face again  ");
        Serial.print(count);
        Serial.print("/2  [");
        preview_queue(&queue);
        Serial.println("]");
