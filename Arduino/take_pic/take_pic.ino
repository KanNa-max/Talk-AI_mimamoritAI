...
// char time_string[20];  // ファイル名に使う文字列のバッファ
SDClass theSD;
...
void loop() {
    ...
    if (take_picture_count < TOTAL_PICTURE_COUNT) {
        ...
        /* Take still picture.
         * Unlike video stream(startStreaming) , this API wait to receive image
        ...
        if (img.isAvailable()) {
            ...
            sprintf(filename, "out/PICT_%s_%03d_%03d.JPG", NAME_KEY, NAME_ID,
                    take_picture_count);
             ...
             * Then, allocate the larger memory size and/or decrease the size of
             * a picture. [How to allocate the larger memory]
